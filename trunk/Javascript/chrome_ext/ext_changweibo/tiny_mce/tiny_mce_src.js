	...
	tinymce.create('tinymce.ui.ListBox:tinymce.ui.Control', {
		...
		select : function(va, current) {
			...
			if (va != t.selectedValue) {
				...
				if (!fv) {
					if (current) {
						DOM.get(t.id + '_text').value = current;
					} else {
						t.selectByIndex(-1);
					}
				}
		...
		selectByIndex : function(idx) {
			...
			if (idx != t.selectedIndex) {
				...
				if (o) {
					...
					e.value = DOM.encode(o.title);
					DOM.removeClass(e, 'mceTitle');
				} else {
					//DOM.setHTML(e, DOM.encode(t.settings.title));
					e.value = DOM.encode(t.settings.title);
		...
		renderHTML : function() {
			...
			h += '<td>' + DOM.createHTML('input', {id : t.id + '_text', 'class' : 'mceText', value : DOM.encode(t.settings.title)}, '') + '</td>';
				...
				if (!t._focused) {
					t.keyDownHandler = Event.add(t.id + '_text', 'keydown', function(e) {
						...
						if (kc == 38)
							...
							v = t.selectedValue;
							switch (t.id.replace(/^.*?_/, '')) {
								case 'fontselect':
									v = e.target.value;
									break;
								case 'fontsizeselect':
									v = {'fontSize': (e.target.value)};
									break;					
							};
	...
	tinymce.create('tinymce.ui.ColorSplitButton:tinymce.ui.SplitButton', {
		...
		renderMenu : function() {
			...
			if (s.more_colors_func) {
				...
				DOM.setHTML(n, '\u989C\u8272\u503C: ');
				n = DOM.add(n, 'input');
				Event.add(n, 'keydown', function(e) {
					switch (e.keyCode) {
						case 13:
							setTimeout(function() {
