...
class WeiboOAuth {
    ...
    function __construct($consumer_key, $consumer_secret, $oauth_token = NULL, $oauth_token_secret = NULL) {
        ...
        } elseif (Session::Get(get_class($this) . '_oauth_token')) {
            $this->token = new OAuthConsumer(Session::Get(get_class($this) . '_oauth_token')
            								,Session::Get(get_class($this) . '_oauth_token_secret'));
        } else {
            $this->token = NULL;
        }
    }
    ...
    function getRequestToken($oauth_callback = NULL, $scope = NULL) {
        ...
        Session::Set(get_class($this) . '_oauth_token', $token['oauth_token']);
        Session::Set(get_class($this) . '_oauth_token_secret', $token['oauth_token_secret']);
//        $_SESSION[get_class($this) . '_oauth_token'] = $token['oauth_token'];
//        $_SESSION[get_class($this) . '_oauth_token_secret'] = $token['oauth_token_secret'];
        return $token;
    }
     ...
     * @return string
     */
    function oAuthRequest($url, $method = 'GET', $parameters = array() , $multi = false) {
        // echo $url ;
        ...
        switch ($method) {
	        case 'GET':
	            //echo $request->to_url();
	            $ret = $this->http($request->to_url(), 'GET');
	            break;
	        default:
	        	//echo $request->to_postdata($multi);
	            $ret = $this->http($request->get_normalized_http_url(), $method, $request->to_postdata($multi) , $multi );
	            break;
        }
	        ...
	        case 'POST':
	            ...
	            if (!empty($postfields)) {
	            	if (is_array($postfields)) {
            			$request = new OAuthRequest($method, $url, $postfields);
            			$postfields = $request->to_postdata($multi);
	            	}
...
class OAuthRequest {
    ...
    public function get_signable_parameters() {
        ...
        return OAuthUtil::build_http_query($params, $this->type);
    }
...
class OAuthUtil {
    ...
    public static function build_http_query($params, $type = NULL) {
        ...
        foreach ($params as $parameter => $value) {
            if (is_array($value)) {
            	...
            	if ($type == 'qzone' && strpos($value, '<') !== false) {
